...
*/
bool	SX1272::isBW(uint16_t band)
{
  #if (SX1272_debug_mode > 1)
	  printf("\n");
...
*/
bool	SX1272::isChannel(uint32_t ch)
{
  #if (SX1272_debug_mode > 1)
	  printf("\n");
...
int8_t SX1272::setPower(char p)
{
  ...
  st0 = readRegister(REG_PA_DAC);
  //printf("power dac set to %x\v", st0);
...
int8_t SX1272::setPacketLength(uint8_t l)
{
	...
	#if (SX1272_debug_mode > 1)
		printf("\n");
		printf("Starting 'setPacketLength' %d\n", l);
	#endif
...
*/
bool	SX1272::availableData(uint16_t wait)
{
	uint8_t value;
	uint8_t header = 0;
	bool forme = false;
	...
	{ // LoRa mode
		...
		{ // header received
			...
			{ // Waiting to read first payload uint8_ts from packet
...
int8_t SX1272::getPacket(uint16_t wait)
{
	...
	if( p_received )
	{
		...
		{
			writeRegister(REG_FIFO_ADDR_PTR, 0x00);  		// Setting address pointer in FIFO data buffer
			packet_received.dst = readRegister(REG_FIFO);	// Storing first uint8_t of the received packet
		}
		else
		{
			value = readRegister(REG_PACKET_CONFIG1);
			...
			{
				packet_received.dst = _destination;			// Storing first uint8_t of the received packet
			}
		}
		packet_received.src = readRegister(REG_FIFO);		// Reading second uint8_t of the received packet
		packet_received.packnum = readRegister(REG_FIFO);	// Reading third uint8_t of the received packet
		packet_received.length = readRegister(REG_FIFO);	// Reading fourth uint8_t of the received packet
		if( _modem == LORA )
		{
...
uint8_t SX1272::setPayload(char *payload)
{
	...
	if( ( _modem == FSK ) && ( _payloadlength > MAX_PAYLOAD_FSK ) )
	{
		...
		state = 1;
		#if (SX1272_debug_mode > 1)
			printf("In FSK, payload length must be less than 60 uint8_ts.\n");
			printf("\n");
		#endif
	}
			...
			if( ACK.src == packet_sent.dst )
			{
				...
				else
				{
					state = 1;
						...
						printf("RECD=%02.2x SENT=%02.2x", ACK.packnum, packet_sent.packnum);
						printf("\n");
					#endif
				}
			}
...
uint8_t SX1272::readRegister(uint8_t address)
{
    ...
    txbuf[0] = address;
	txbuf[1] = 0x00;
	bcm2835_spi_transfernb(txbuf, rxbuf, 2);
